...
function addSheet(no, name, cover) {
    ...
    // 仅对本地歌单（id 以 pl_ 开头）显示删除按钮
    var delBtn = '';
    if(musicList[no] && typeof musicList[no].id === 'string' && musicList[no].id.indexOf('pl_') === 0) {
        delBtn = '<span class="sheet-del" title="删除歌单">×</span>';
    }
...
// 为歌单列表绑定删除按钮事件（一次绑定，事件委托到 #sheet）
function bindSheetDelete() {
    $('#sheet').off('click.sheetdel').on('click.sheetdel', '.sheet-del', function(e) {
        e.stopPropagation();   // 不触发歌单的点击播放
        var no = $(this).closest('.sheet-item').data('no');
        var id = musicList[no] ? musicList[no].id : '';
        if(!id || id.indexOf('pl_') !== 0) { layer.msg('只能删除本地歌单'); return; }
        ajaxDeletePlayList(id, no);
    });
}
// 清空歌单显示
function clearSheet() {
...
// 初始化播放列表
function initList() {
    bindSheetDelete();   // 绑定歌单删除按钮事件（内部用 off+on，重复调用安全）
